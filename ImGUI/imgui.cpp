...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(2, 2), grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetColorU32(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDING (THIS IS COMBO ARROW)
																																															  //window->DrawList->AddLine(ImVec2(), ImVec2(), ImColor(240, 240, 240, 240), 1.f);
																																															  //RenderCollapseTriangle(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y) + style.FramePadding, true);
	...
	if (!IsPopupOpen(id))
	{
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - (arrow_size / 2) - 0.5, frame_bb.Min.y + 3.5), ImVec2(frame_bb.Max.x - (arrow_size / 2) - 0.5, frame_bb.Max.y - 3.5), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 6, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Max.x - 6, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
	}
	if (IsPopupOpen(id))
	{
		window->DrawList->AddLine(ImVec2(frame_bb.Max.x - arrow_size + 6, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImVec2(frame_bb.Max.x - 6, frame_bb.Min.y + ((frame_bb.Max.y - frame_bb.Min.y) / 2)), ImColor(0.95f, 0.95f, 0.95f, style.Alpha), 2.f);
